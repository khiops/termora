...
import { type AuthPromptFn, buildSshConnectConfig, SshAgent, type SshAgentDeployOptions } from './ssh-agent.js';
...
export class SshConnectionManager {
	...
	buildPromptAuth(client: WsClient): AuthPromptFn {
		return async (hostId, promptType, message) => {
			...
			if (promptType === 'passphrase') {
				...
				console.error(`[termora-ssh] passphrase cache lookup for ${hostId}: ${cached ? 'hit' : 'miss'}`);
				if (cached) {
					if (cached.expiresAt > Date.now()) {
						console.error(`[termora-ssh] returning cached passphrase (length ${cached.secret.length})`);
	...
	scheduleReconnect(hostId: string, sessionId: string, attemptIndex: number, startTime: number): void {
		...
		const delayMs = RECONNECT_BACKOFF_MS[Math.min(attemptIndex, RECONNECT_BACKOFF_MS.length - 1)] ?? 30_000;
