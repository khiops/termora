	...
	return makeMockClient(
		{
			...
			'test -x "/usr/bin/termora-agent" && echo "/usr/bin/termora-agent"': { stdout: '', stderr: '', exitCode: 1 },
...
describe('uploadAgentBinary', () => {
	it('calls mkdir, fastPut, and chmod in order', async () => {
		...
		expect(sftp.chmod).toHaveBeenCalledWith('/remote/.local/bin/termora-agent', 0o755, expect.any(Function));
	});
	...
	it('calls sftp.end() even when fastPut fails', async () => {
		...
		await expect(uploadAgentBinary(client, '/local/binary', '/remote/termora-agent')).rejects.toThrow('disk full');
	...
	it('rejects when sftp channel open fails', async () => {
		...
		await expect(uploadAgentBinary(client, '/local/binary', '/remote/termora-agent')).rejects.toThrow(
			'SFTP not available',
		);
	});
	...
	it('handles Windows backslash paths for parent dir extraction', async () => {
		...
		await uploadAgentBinary(client, 'C:\\local\\termora-agent.exe', '%LOCALAPPDATA%\\termora\\termora-agent.exe');
...
describe('deployAgentIfNeeded — agent already present', () => {
	...
	it('2. SHA256 mismatch (local cache) → re-upload, onAgentUpdated called', async () => {
		...
		const result = await deployAgentIfNeeded(client, { os: 'linux', arch: 'x64' }, makeOptions({ onAgentUpdated }));
	...
	it('4. No local binary, no pin → calls promptBinaryVerify', async () => {
		...
		await deployAgentIfNeeded(client, { os: 'linux', arch: 'x64' }, makeOptions({ promptBinaryVerify }));
...
describe('deployAgentIfNeeded — agent not found', () => {
	...
	it('13. Agent not found + no local binary → throws AGENT_NOT_AVAILABLE', async () => {
		...
		const error = await deployAgentIfNeeded(client, { os: 'linux', arch: 'x64' }, makeOptions()).catch(
			(e: unknown) => e,
		);
	...
	it('throws when OS/arch cannot be detected and they are unknown', async () => {
		...
		await expect(deployAgentIfNeeded(client, { os: null, arch: null }, makeOptions())).rejects.toThrow(
...
describe('deploy + verify integration', () => {
	it('deploys from cache, then skips on second connect (SHA256 match)', async () => {
		...
		const resultFirst = await deployAgentIfNeeded(clientFirst, { os: 'linux', arch: 'x64' }, makeOptions());
		...
		const resultSecond = await deployAgentIfNeeded(clientSecond, { os: 'linux', arch: 'x64' }, makeOptions());
