...
export function registerHostCrudRoutes(server: FastifyInstance, metaDal: MetaDAL): void {
	...
	server.get<{ Querystring: { limit?: string; offset?: string } }>('/api/hosts', async (request) => {
	...
	server.post<{ Body: CreateHostBody }>('/api/hosts', async (request, reply) => {
		...
		if (existing) {
			return reply.code(409).send({ error: { code: 'CONFLICT', message: 'Host with this label already exists' } });
		}
	...
	for (const url of ['/api/hosts/order', '/api/hosts/reorder']) {
		server.put<{ Body: { group_id: string | null; host_ids: string[] } }>(url, async (request, reply) => {
	...
	// PUT /api/hosts/:id
	server.put<{ Params: { id: string }; Body: UpdateHostBody }>('/api/hosts/:id', async (request, reply) => {
		const host = metaDal.getHost(request.params.id);
		if (!host) {
			return reply.code(404).send({ error: { code: 'NOT_FOUND', message: 'Host not found' } });
		}
		...
		if (body.label !== undefined) {
			...
			if (!/^[a-zA-Z0-9._-]+$/.test(body.label)) {
				return reply.code(400).send({
					error: {
						code: 'VALIDATION_ERROR',
						message: 'Label must contain only alphanumeric characters, dots, dashes, and underscores',
					},
				});
			}
		...
		if (body.trust_remote_hints !== undefined) updateInput.trustRemoteHints = body.trust_remote_hints;
		...
		if (body.keep_alive_seconds !== undefined) updateInput.keepAliveSeconds = body.keep_alive_seconds;
		if (body.history_retention_days !== undefined) updateInput.historyRetentionDays = body.history_retention_days;
