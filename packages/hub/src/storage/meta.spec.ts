...
describe("MetaDAL — countChannelsForSession", () => {
	let dbs: DatabaseManager;
	let dal: MetaDAL;
	...
	it("counts channels for a session", () => {
		const host = dal.createHost({ type: "local", label: "count-host" });
		const sessionId = "COUNTSESS01AAAAAAAAAAAAAAAAAA";
		dal.createSession({ id: sessionId, hostId: host.id, status: "active" });

		expect(dal.countChannelsForSession(sessionId)).toBe(0);

		dal.createChannel({ id: "COUNTCH01AAAAAAAAAAAAAAAAAAA", sessionId, status: "born" });
		dal.createChannel({ id: "COUNTCH02AAAAAAAAAAAAAAAAAAA", sessionId, status: "born" });

		expect(dal.countChannelsForSession(sessionId)).toBe(2);
	});

	it("does not count channels from other sessions", () => {
		const host = dal.createHost({ type: "local", label: "count-host-2" });
		const sessionId1 = "COUNTSESS02AAAAAAAAAAAAAAAAAA";
		const sessionId2 = "COUNTSESS03AAAAAAAAAAAAAAAAAA";
		dal.createSession({ id: sessionId1, hostId: host.id, status: "active" });
		dal.createSession({ id: sessionId2, hostId: host.id, status: "active" });

		dal.createChannel({
			id: "COUNTCH03AAAAAAAAAAAAAAAAAAA",
			sessionId: sessionId1,
			status: "born",
		});
		dal.createChannel({
			id: "COUNTCH04AAAAAAAAAAAAAAAAAAA",
			sessionId: sessionId2,
			status: "born",
		});
		dal.createChannel({
			id: "COUNTCH05AAAAAAAAAAAAAAAAAAA",
			sessionId: sessionId2,
			status: "born",
		});

		expect(dal.countChannelsForSession(sessionId1)).toBe(1);
		expect(dal.countChannelsForSession(sessionId2)).toBe(2);
	});
});

describe("MetaDAL — maxChannelNumberForSession", () => {
	let dbs: DatabaseManager;
	let dal: MetaDAL;
	...
	it("returns 0 when no channels exist", () => {
		const host = dal.createHost({ type: "local", label: "max-host" });
		const sessionId = "MAXSESS001AAAAAAAAAAAAAAAAAA";
		dal.createSession({ id: sessionId, hostId: host.id, status: "active" });
		expect(dal.maxChannelNumberForSession(sessionId)).toBe(0);
	});

	it("returns highest Shell #N number", () => {
		const host = dal.createHost({ type: "local", label: "max-host-2" });
		const sessionId = "MAXSESS002AAAAAAAAAAAAAAAAAA";
		dal.createSession({ id: sessionId, hostId: host.id, status: "active" });

		dal.createChannel({
			id: "MAXCH001AAAAAAAAAAAAAAAAAAAA",
			sessionId,
			status: "born",
			title: "Shell #1",
		});
		dal.createChannel({
			id: "MAXCH002AAAAAAAAAAAAAAAAAAAA",
			sessionId,
			status: "born",
			title: "Shell #5",
		});
		dal.createChannel({
			id: "MAXCH003AAAAAAAAAAAAAAAAAAAA",
			sessionId,
			status: "born",
			title: "Shell #3",
		});

		expect(dal.maxChannelNumberForSession(sessionId)).toBe(5);
	});

	it("ignores non-Shell titles", () => {
		const host = dal.createHost({ type: "local", label: "max-host-3" });
		const sessionId = "MAXSESS003AAAAAAAAAAAAAAAAAA";
		dal.createSession({ id: sessionId, hostId: host.id, status: "active" });

		dal.createChannel({
			id: "MAXCH004AAAAAAAAAAAAAAAAAAAA",
			sessionId,
			status: "born",
			title: "Shell #2",
		});
		dal.createChannel({
			id: "MAXCH005AAAAAAAAAAAAAAAAAAAA",
			sessionId,
			status: "born",
			title: "btop",
		});

		expect(dal.maxChannelNumberForSession(sessionId)).toBe(2);
	});

	it("includes dead channels in the count", () => {
		const host = dal.createHost({ type: "local", label: "max-host-4" });
		const sessionId = "MAXSESS004AAAAAAAAAAAAAAAAAA";
		dal.createSession({ id: sessionId, hostId: host.id, status: "active" });

		dal.createChannel({
			id: "MAXCH006AAAAAAAAAAAAAAAAAAAA",
			sessionId,
			status: "born",
			title: "Shell #3",
		});
		dal.updateChannelStatus("MAXCH006AAAAAAAAAAAAAAAAAAAA", "dead");

		expect(dal.maxChannelNumberForSession(sessionId)).toBe(3);
	});
});
