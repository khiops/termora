...
describe('buildStaticManifest', () => {
	...
	it('produces a valid manifest with correct structure', async () => {
		...
		const manifest = JSON.parse(manifestJson) as Record<string, { data: string; contentType: string }>;
	...
	it('assigns correct content-types to each file', async () => {
		...
		const manifest = JSON.parse(buildStaticManifest(tempDir)) as Record<string, { data: string; contentType: string }>;
...
describe('hub SEA binary build integration', () => {
	const SEA_BINARY = join(ROOT, 'dist', 'sea', process.platform === 'win32' ? 'termora-hub.exe' : 'termora-hub');

	it('binary exists after build (or build produces valid CJS at minimum)', { timeout: 120_000 }, () => {
...
describe('sea-agent-resolver', () => {
	...
	it('finds agent binary in the same directory as hub binary', async () => {
		const { resolveAgentBinaryPath, _AGENT_BINARY_NAME } = await import('../packages/hub/src/sea-agent-resolver.js');
