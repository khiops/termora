...
describe("buildStaticManifest", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `nexterm-manifest-test-${Date.now()}`);
...
describe("hub SEA binary build integration", () => {
	const SEA_BINARY = join(
		...
		"dist",
		"sea",
		process.platform === "win32" ? "nexterm-hub.exe" : "nexterm-hub",
	);

	it(
		"binary exists after build (or build produces valid CJS at minimum)",
		{ timeout: 120_000 },
		() => {
			const cjsBundle = join(ROOT, "dist", "sea", "nexterm-hub.cjs");
			...
			if (!existsSync(cjsBundle)) {
				console.log("[test] nexterm-hub.cjs not built yet — skipping SEA binary check");
				return;
			}
...
describe("sea-agent-resolver", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `nexterm-resolver-test-${Date.now()}`);
		mkdirSync(tempDir, { recursive: true });
	});
	...
	it("returns null when agent binary does not exist anywhere", async () => {
		...
		const impossibleName = `nexterm-agent-does-not-exist-${Date.now()}`;
