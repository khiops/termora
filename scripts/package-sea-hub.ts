...
const effectivePlatform =
	...
	process.platform;
const effectiveArch = targetArchArg ?? tripleToNodeArch(process.env.TERMORA_TARGET_TRIPLE) ?? process.arch;
const effectiveNodeVersion = targetNodeVersionArg ?? process.env.TERMORA_NODE_VERSION ?? process.version;
...
export function locateTomlWasm(): string {
	...
	const rootNodeModules = join(ROOT, 'node_modules', '@rainbowatcher', 'toml-edit-js', 'index_bg.wasm');
...
export function locateBetterSqlite3(): string {
	...
	// Fallback: root-level node_modules (hoisted installs / npm/yarn)
	const rootNodeModules = join(ROOT, 'node_modules', 'better-sqlite3', 'build', 'Release', 'better_sqlite3.node');
	if (existsSync(rootNodeModules)) {
		return rootNodeModules;
	}
