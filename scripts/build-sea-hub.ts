 ...
 * build-sea-hub.ts
 *
 * esbuild bundler for the nexterm-hub Node SEA binary.
 *
 * Bundles packages/hub/src/cli.ts into a single CJS file at
 * dist/sea/nexterm-hub.cjs suitable for embedding in a Node SEA blob.
 *
 * Key decisions:
...
const SEA_BANNER_LINES = [
	...
	"    }",
	"  } catch (err) {",
	"    process.stderr.write('[nexterm-hub] fatal: cannot extract better_sqlite3.node: ' + err + '\\n');",
	"    process.exit(1);",
	"  }",
	...
	"  } catch (err) {",
	"    process.stderr.write('[nexterm-hub] fatal: dlopen better_sqlite3.node failed: ' + err + '\\n');",
	"    process.exit(1);",
	"  }",
