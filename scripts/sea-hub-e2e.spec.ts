 ...
 * sea-hub-e2e.spec.ts
 *
 * End-to-end tests that validate the nexterm-hub SEA binary works correctly.
 *
 * Prerequisites:
 *   pnpm run package:sea-hub   (builds dist/sea/nexterm-hub)
 *
...
const SEA_BINARY = join(SEA_DIR, process.platform === "win32" ? "nexterm-hub.exe" : "nexterm-hub");
const AGENT_BINARY = join(
	SEA_DIR,
	process.platform === "win32" ? "nexterm-agent.exe" : "nexterm-agent",
);
...
/**
 * Spawn the nexterm-hub SEA binary in foreground mode on a random port,
...
async function startHub(extraEnv: Record<string, string> = {}): Promise<HubProcess> {
	...
	const stateDir = mkdtempSync(join(tmpdir(), "nexterm-hub-state-"));
	const configDir = mkdtempSync(join(tmpdir(), "nexterm-hub-config-"));
...
function readAuthToken(configDir: string): string {
	// configDir is XDG_CONFIG_HOME; hub appends "nexterm/auth.json"
	const authPath = join(configDir, "nexterm", "auth.json");
...
describe("nexterm-hub SEA binary E2E", { timeout: 120_000 }, () => {
	...
	it.skipIf(!RUN_SEA_TESTS)(
		...
		async () => {
			...
			const authPath = join(hub.configDir, "nexterm", "auth.json");
	...
	it.skipIf(!RUN_SEA_TESTS)(
		...
		async () => {
			...
			const metaDb = join(hub.stateDir, "nexterm", "meta.db");
			const spoolDb = join(hub.stateDir, "nexterm", "spool.db");
