...
/// Linux/macOS: $XDG_STATE_HOME/termora/logs/agent-daemon.jsonl
/// Windows: %LOCALAPPDATA%\termora\logs\agent-daemon.jsonl
...
mod tests {
    ...
    fn daemon_log_path_returns_jsonl() {
        ...
        assert!(path.to_string_lossy().contains("termora"));
    ...
    fn daemon_log_path_xdg_state_home_respected() {
        ...
        let tmp = std::env::temp_dir().join("termora-test-logging-state");
