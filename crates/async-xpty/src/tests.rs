...
async fn test_read_output() {
	let mut pty =
		CommandBuilder::new("/bin/sh").arg("-c").arg("echo hello").spawn().await.unwrap();
...
async fn test_write_input() {
	...
	let output = String::from_utf8_lossy(&buf[..n]);
	assert!(output.contains("test123"), "expected 'test123' in {:?}", output);
...
async fn test_resize() {
	let pty = CommandBuilder::new("/bin/sh").spawn().await.unwrap();
	pty.resize(PtySize { cols: 120, rows: 40 }).await.unwrap();
	pty.kill().ok();
}
...
async fn test_exit_code() {
	let mut pty =
		CommandBuilder::new("/bin/sh").arg("-c").arg("exit 42").spawn().await.unwrap();
	...
	assert_eq!(status.code(), Some(42), "expected exit code 42, got {:?}", status);
}
...
async fn test_ctrl_c_signal() {
	let mut pty =
		CommandBuilder::new("/bin/sh").arg("-c").arg("sleep 60").spawn().await.unwrap();
