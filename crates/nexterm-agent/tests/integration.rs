...
async fn read_frame(stdout: &mut ChildStdout) -> rmpv::Value {
	...
	stdout.read_exact(&mut len_buf).await.expect("read length header");
...
async fn test_hello_on_startup() {
	...
	assert_eq!(hello["type"].as_str(), Some("HELLO"), "first frame must be HELLO");
	assert_eq!(hello["version"].as_u64(), Some(1), "version must be 1");
	assert!(hello["capabilities"].is_array(), "capabilities must be an array");
...
async fn test_heartbeat_ack() {
	...
	assert_eq!(ack["ts"].as_str(), Some(ts), "ts must be echoed back verbatim");
...
async fn test_spawn_ok() {
	...
	let ch_id = resp["channel_id"].as_str().expect("channel_id must be present in SPAWN_OK");
...
async fn test_full_lifecycle() {
	...
	loop {
		...
		let per_frame_ms = if exit_grace_deadline.is_some() { 500 } else { 2000 };
		let frame =
	...
	assert!(saw_output, "expected OUTPUT frame containing 'lifecycle_test'");
...
async fn test_multiple_heartbeats_in_order() {
	...
	for i in 0u32..3 {
		...
		let ack = read_frame_timeout(&mut stdout, 5).await;
		assert_eq!(ack["type"].as_str(), Some("HEARTBEAT_ACK"), "heartbeat {i} must be acked");
		assert_eq!(ack["ts"].as_str(), Some(ts.as_str()), "ts must match for heartbeat {i}");
	}
