...
pub(crate) async fn handle_message(
	...
	match msg {
		...
		HubToAgent::Resize { channel_id, cols, rows } => {
...
async fn handle_spawn(
	...
	let expanded_args: Vec<String> =
		args.iter().map(|a| expand_vars(a, env.as_ref())).collect();
	let expanded_cwd: Option<String> = cwd.map(|d| expand_vars(&d, env.as_ref()));
	let expanded_env: Option<std::collections::HashMap<String, String>> =
		env.map(|e| e.into_iter().map(|(k, v)| (k, expand_vars(&v, None))).collect());

	// Determine effective program + args (may be wrapped by elevation)
	let (effective_program, effective_args, extra_env, cleanup_path) =
		if elevated.unwrap_or(false) {
						...
						None => {
							send_frame(
								...
								&AgentToHub::SpawnErr {
									...
									message: "custom elevation method requires custom_command field"
										.into(),
								},
							)?;
							return Ok(());
			...
			let secret: Option<Zeroizing<String>> =
				elevation_secret.map(|s| Zeroizing::new(s));
	...
	match spawn_result {
		Ok((ch_id, pty_pid)) => {
			...
			send_frame(&frame_tx, &AgentToHub::SpawnOk { request_id, channel_id: ch_id })?;
		}
