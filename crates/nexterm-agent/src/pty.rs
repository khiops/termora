...
impl PtyManager {
	...
	pub async fn spawn(
		...
		let pid = process.pid();
		self.channels.insert(id.clone(), PtyChannel { process, seq: 0 });
		Ok((id, pid))
	}
...
mod tests {
	...
	async fn test_spawn_channel() {
		...
		let (id, pid) = mgr.spawn(None, "/bin/sh", &[], None, None, 80, 24).await.unwrap();
		assert!(!id.is_empty());
		assert!(pid > 0);
	...
	async fn test_destroy_all() {
		let mut mgr = PtyManager::new();
		mgr.spawn(None, "/bin/sh", &[], None, None, 80, 24).await.unwrap();
		mgr.spawn(None, "/bin/sh", &[], None, None, 80, 24).await.unwrap();
