...
async fn read_auth_token(config_dir: &str) -> Option<String> {
	...
	match serde_json::from_str::<serde_json::Value>(&content) {
		Ok(v) => v
			...
			.or_else(|| {
				tracing::error!(
					"auth.json missing 'token' field — connections will be rejected"
				);
...
async fn validate_auth<R: AsyncRead + Unpin>(
	...
	.await
	.map_err(|_| {
		std::io::Error::new(std::io::ErrorKind::TimedOut, "AUTH frame not received within 5s")
	})??;

	// Match the decoded message — only HubToAgent::Auth passes
	match first_msg {
		HubToAgent::Auth { token } => {
			Ok(ct_eq(expected_token.as_bytes(), token.as_bytes()))
		}
		_other => {
...
mod tests {
	...
	async fn test_read_auth_token_malformed() {
		...
		tokio::fs::create_dir_all(&dir).await.unwrap();
		let auth_path = dir.join("auth.json");
		tokio::fs::write(&auth_path, b"not json at all").await.unwrap();
	...
	async fn test_read_auth_token_missing_token_field() {
		...
		tokio::fs::create_dir_all(&dir).await.unwrap();
		let auth_path = dir.join("auth.json");
		tokio::fs::write(&auth_path, br#"{"other":"value"}"#).await.unwrap();
	...
	async fn test_daemon_accepts_correct_auth_token() {
		...
		assert!(n2 >= 4, "must receive at least 4-byte frame header after auth");
