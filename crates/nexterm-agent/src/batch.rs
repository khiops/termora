...
mod tests {
	...
	async fn test_batch_flushes_on_timer() {
		...
		out_tx
			.send(OutputEvent { channel_id: "ch1".into(), seq: 1, data: b"hello".to_vec() })
			.unwrap();
	...
	async fn test_batch_flushes_on_threshold() {
		...
		out_tx
			.send(OutputEvent { channel_id: "ch2".into(), seq: 5, data: big_data })
			.unwrap();

		tokio::time::sleep(std::time::Duration::from_millis(10)).await;

		let batched = batch_rx.try_recv().expect("should have flushed on threshold");
